...
pub fn encrypt_aes256_cbc(data: &[u8], key: &[u8; 32]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    let mut iv = [0u8; 16];
...
pub fn decrypt_aes256_cbc(encrypted_data: &[u8], key: &[u8; 32]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
